			   ...
			   TEE_UUID **uuids, size_t *count)
{
	...
	if (get_persistent_objects_list(session->token, NULL, &size) !=
			SKS_SHORT_BUFFER)
		return SKS_ERROR;
				  ...
				  TEE_Param *in, TEE_Param *out)
{
	...
	idx = session->find_ctx->next;
	if (idx < session->find_ctx->temp_start)
		idx = session->find_ctx->temp_start;

	for (;idx < session->find_ctx->count; idx++)
		release_object_from_handle(session->find_ctx->handles[idx]);

	TEE_Free(session->find_ctx->attributes);
	TEE_Free(session->find_ctx->handles);
	TEE_Free(session->find_ctx);
