...
static enum tpm2_result tpm2_get_locality(struct tpm2_chip *chip, int loc)
{
	...
	/* if not get one */
	DMSG("t_cnt = %lu, to = %lu", t_cnt, chip->timeout_a);
	...
	EMSG("Timeout t_cnt = %lu ms", t_cnt);
					     ...
					     uint8_t *status)
{
	...
	DMSG("t_cnt = %lu, to = %lu", t_cnt, timeout);
	do {
...
enum tpm2_result tpm2_init(struct tpm2_chip *chip)
{
	...
	/*
	 * chip->timeout_a start out as 0 in
	 * tpm2_get_locality()
	 */
...
enum tpm2_result tpm2_rx(struct tpm2_chip *chip, uint8_t *buf, size_t len)
{
	...
	size = tpm2_rx_dat(chip, buf, TPM2_HDR_LEN);
	if (size < TPM2_HDR_LEN) {
