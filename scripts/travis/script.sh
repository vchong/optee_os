...
if [ "$TRAVIS_PULL_REQUEST" == "false" ] && [ "${COVERITY_SCAN_BRANCH}" != 1 ]
then
	checkpatch HEAD
elif [ "${COVERITY_SCAN_BRANCH}" != 1 ]
then
...
if [ "${COVERITY_SCAN_BRANCH}" == 1 ]; then
	cat /home/travis/build/vchong/optee_os/cov-int/build-log.txt
	cat /home/travis/build/vchong/optee_os/cov-int/scm_log.txt
fi
